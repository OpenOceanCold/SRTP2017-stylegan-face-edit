...
def move_latent_and_save(latent_vector, direction_file, coeffs, Gs_network, Gs_syn_kwargs):
    ...
    os.makedirs('results/'+direction_file.split('.')[0] + "3", exist_ok=True)
    ...
    for i, coeff in enumerate(coeffs):
        ...
        result.save('results/'+direction_file.split('.')[0] + "3" +'/'+str(i).zfill(3)+'.png')
...
def main():
    ...
    '''
        ...
        width.npy - 调整脸的宽度
    '''
    direction_file = 'height.npy'  # 从上面的编辑向量中选择一个

    # 在这儿选择调整的大小，向量里面的值表示调整幅度，可以自行编辑，对于每个值都会生成一张图片并保存。
    coeffs = [0 for x in range(0, 45)] 
    for i in range(len(coeffs)):
        coeffs[i] = -15 + (i-1)*0.67 
    # 开始调整并保存图片
